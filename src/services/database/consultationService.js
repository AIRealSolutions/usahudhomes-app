...
export default consultationService
   ...
   * Get broker statistics
   * @param {string} brokerId - Broker UUID
   * @returns {Promise<Object>} Broker statistics
   */
  async getBrokerStats(brokerId) {
    try {
      // Use the broker_stats view if available, otherwise calculate manually
      const { data, error } = await supabase
        .from('broker_stats')
        .select('*')
        .eq('broker_id', brokerId)
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 = no rows returned
        console.error('Error fetching broker stats from view:', error)
        // Fallback to manual calculation
        return await this.calculateBrokerStats(brokerId)
      }
  ...
  /**
   * Calculate broker statistics manually (fallback)
   * @param {string} brokerId - Broker UUID
   * @returns {Promise<Object>} Calculated statistics
   */
  async calculateBrokerStats(brokerId) {
    try {
      ...
      const stats = {
        pending_referrals: consultations.filter(c => c.status === 'referred').length,
        accepted_leads: consultations.filter(c => c.status === 'accepted').length,
        active_leads: consultations.filter(c => ['pending', 'scheduled'].includes(c.status)).length,
        completed_this_month: consultations.filter(c => 
          c.status === 'completed' && new Date(c.created_at) >= thirtyDaysAgo
        ).length,
        declined_total: consultations.filter(c => c.status === 'declined').length,
        expired_total: consultations.filter(c => c.status === 'expired').length,
        total_emails_sent: consultations.reduce((sum, c) => sum + (c.email_count || 0), 0),
        total_sms_sent: consultations.reduce((sum, c) => sum + (c.sms_count || 0), 0),
        total_calls_made: consultations.reduce((sum, c) => sum + (c.call_count || 0), 0)
      }

      // Calculate rates
      const totalProcessed = stats.accepted_leads + stats.declined_total + stats.expired_total
      stats.acceptance_rate = totalProcessed > 0 
        ? Math.round((stats.accepted_leads / totalProcessed) * 100) 
        : 0

      const totalCompleted = consultations.filter(c => c.status === 'completed').length
      const totalWon = consultations.filter(c => c.status === 'completed' && c.outcome === 'won').length
      stats.conversion_rate = totalCompleted > 0 
  ...
  async updateStatus(consultationId, newStatus) {
    try {
      ...
      if (newStatus === 'scheduled' && !updateData.scheduled_at) {
        updateData.scheduled_at = new Date().toISOString()
      }
      if (newStatus === 'completed' && !updateData.completed_at) {
