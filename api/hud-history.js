  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (error) {
        ...
        return res.status(200).json({ success: true, runs: [], warning: 'Run migration to enable history: database/migrations/add_hud_sync_tables.sql' })
      }
    ...
    if (req.method === 'POST' && req.query?.action === 'queue-media') {
      ...
      if (!state && (!case_numbers || case_numbers.length === 0)) {
        return res.status(400).json({ success: false, error: 'state or case_numbers required' })
      }
      ...
      if (case_numbers && case_numbers.length > 0) {
        ...
        query = query.eq('state', state.toUpperCase()).eq('status', 'AVAILABLE')
      }
      ...
      let resolvedTemplate = template_id
      if (!resolvedTemplate) {
      ...
      const rows = props.map(p => ({
        property_id:  p.id,
        template_id:  resolvedTemplate || null,
